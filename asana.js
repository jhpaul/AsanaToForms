...
//		Logger.log(each)
		...
		if (each === "memberships") {
			if (task[each][0]["project"] === "" && task[each][0][
					"section"
				] === "") {
				delete task[each]
			}
...
//		var taskJSON = JSON.parse(result.getContentText());
      var taskJSON = callAsanaApi("POST", "tasks", JSON.stringify({"data":task}))
//		Logger.log(task);

		var url = "https://app.asana.com/0/0/" + taskJSON[
			"id"
		] + "/f"
...
//            if (each.columnName === undefined || each.columnName === null || each.columnName ===
            if (each.dueDateDuration !== "" && (each.dueDateDuration !== undefined || each.dueDateDuration !==null)){
